...
const MeetingIdForm = ({ id, meetingAction, buttonText, meetingStatus }) => {
...
const mapStateToProps = (state) => ({
  id: state.app.controlPanel.id,
  meetingAction:
    state.app.controlPanel.callback ||
    (() => {
      console.log("MeetingIdForm default callback clicked");
    }),
  buttonText: state.app.controlPanel.data.buttonText || "Akce",
  meetingStatus:
    state.app.controlPanel.data.meetingStatus || MEETING_STATUSES.UNKNOWN,
});

export default connect(mapStateToProps)(MeetingIdForm);
